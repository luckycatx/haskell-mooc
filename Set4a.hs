...
longest :: Ord a => [[a]] -> [a]
longest = maximumBy (comparing (\xs -> (length xs, Down $ head xs)))
...
incrementKey = map . (\k (k', v) -> if k == k' then (k, v + 1) else (k', v))
...
winner scores player1 player2 = if score player1 >= score player2 then player1 else player2
  where score = flip (Map.findWithDefault 0) scores
...
transfer from to amount bank
  ...
  | otherwise = Map.adjust (subtract amount) from $ Map.adjust (+ amount) to bank
