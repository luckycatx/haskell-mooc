...
add x (Set xs) = Set (sort (nub (x:xs)))
...
data State = Start | Eggs | Flour | Sugar | Ready2Mix | Mixed | Error | Finished
  deriving (Eq,Show)
...
step Flour AddSugar = Ready2Mix
step Sugar AddFlour = Ready2Mix
step Ready2Mix Mix = Mixed
...
passwordAllowed s (ContainsSome cs) = any (`elem` cs) s
passwordAllowed s (DoesNotContain cs) = not (any (`elem` cs) s)
