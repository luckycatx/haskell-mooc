...
visit maze place = do
  ...
  if place `elem` visited
    then return ()
      ...
      put (place:visited)
      mapM_ (visit maze) (maybe [] id (lookup place maze))
...
allSums :: [Int] -> [Int]
allSums xs = do
  bs <- replicateM (length xs) [True,False]
  return $ sum $ zipWith (\b x -> if b then x else 0) bs xs
...
instance Functor SL where
  -- implement fmap
  fmap f (SL g) = SL (\s -> let (a,s',msgs) = g s in (f a,s',msgs))
