...
prettyPrint n cs = unlines [[if (i,j) `elem` cs then 'Q' else '.' | j <- [1..n]] | i <- [1..n]]
...
prettyPrint2 n qs = unlines [[if (i,j) `elem` qs then 'Q' else if danger (i,j) qs then '#' else '.' | j <- [1..n]] | i <- [1..n]]
